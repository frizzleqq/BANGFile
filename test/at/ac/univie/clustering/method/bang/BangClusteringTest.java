...
public class BangClusteringTest {
	...
	public void testMapRegion() {
		...
		BangClustering bang = new BangClustering(4);
		int[] level = {4, 3, 2, 1, 0};
		int[] grid = {4, 5, 6, 7, 8};
		bang.setLevels(level);
		bang.setGrids(grid);
		
		float[] tuple_buf = {0.2f, 0.4f, 0.6f, 0.5f};
		assertEquals(4, bang.mapRegion(tuple_buf));
		
		tuple_buf[0] = 0.7f;
		assertEquals(5, bang.mapRegion(tuple_buf));
		
		tuple_buf[0] = 0.9f;
		assertEquals(13, bang.mapRegion(tuple_buf));
		
		level[0] = 1;
		grid[0] = 1;
		assertEquals(1, bang.mapRegion(tuple_buf));
	}
